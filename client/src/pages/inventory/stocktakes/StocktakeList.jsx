...
export default function StocktakeList() {
  ...
  const { stocktakes, loading, error, fetchStocktakes, confirm, cancel } = useInventoryStocktake();
  ...
  const stats = useMemo(() => {
    return {
      ...
      draft: stocktakes.filter((r) => r.status === 'DRAFT').length,
      confirmed: stocktakes.filter((r) => r.status === 'CONFIRMED').length,
    };
  }, [stocktakes]);

  const handleConfirm = async (e, id) => {
    e.stopPropagation();
    if (!window.confirm('Xác nhận phiếu kiểm kê? Tồn kho sẽ được cập nhật theo kết quả kiểm kê.')) return;
    try {
      await confirm(id);
  ...
  const handleCancel = async (e, id) => {
    e.stopPropagation();
    const r = stocktakes.find((x) => x.id === id);
    if (!r) return;
    if (!window.confirm(`Hủy phiếu kiểm kê ${r.code}?`)) return;
    try {
      await cancel(id);
      ...
      <div className="flex flex-col gap-4 w-full min-w-0">
        <div className="flex flex-row items-start justify-between gap-4">
          <div className="min-w-0">
            <h2 className="text-xl font-bold">Kiểm kê kho</h2>
            <p className="text-muted text-sm">Quản lý các phiếu kiểm kê tồn kho nguyên liệu</p>
          </div>
          <button
            ...
            <ResponsiveTable>
              <thead>
                <tr>
                  ...
                  <th className="hidden md:table-cell">Tổng chênh lệch</th>
              ...
              <tbody>
                {filtered.map((r) => {
                  ...
                  const totalDiff = r.items?.reduce((s, i) => s + (parseFloat(i.difference) || 0), 0) || 0;
                  return (
                    <tr
                      key={r.id}
                    ...
                    >
                      ...
                      <td className="whitespace-nowrap">{fmtDate(r.stocktakeDate)}</td>
                      ...
                      <td className="text-right whitespace-nowrap">
                        <div className="flex items-center justify-end gap-1">
                          <button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              navigate(`/inventory/stocktakes/${r.id}`);
                            }}
                            title="Xem chi tiết"
                          >
                            <Eye size={16} />
                          ...
                          {r.status === 'DRAFT' && (
                            <>
                              <button
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  navigate(`/inventory/stocktakes/${r.id}/edit`);
                                }}
                                title="Sửa"
                              >
                                <Edit3 size={16} />
                ...
                {filtered.length === 0 && !loading && (
                  <tr>
                    <td colSpan={7} className="text-center text-muted py-8">
                      Không tìm thấy phiếu kiểm kê kho
                    </td>
                  </tr>
                )}
              </tbody>
