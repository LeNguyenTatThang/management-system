...
export default function StocktakeCreate() {
  ...
  const { addStocktake } = useInventoryStocktake();
  ...
  const [items, setItems] = useState([
    { ingredientId: '', unitId: '', actualQuantity: '', currentStock: 0, note: '' },
  ]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success('Đã tạo phiếu kiểm kê kho');
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-2 text-sm text-muted">
            ...
            <span>&gt;</span>
            <span className="text-main font-semibold">Tạo phiếu kiểm kê</span>
          </div>
          <button
        ...
        <div className="mb-6">
          <h1 className="text-2xl font-bold">Tạo phiếu kiểm kê kho</h1>
          <p className="text-muted text-sm mt-1">Kiểm tra và đối chiếu tồn kho hệ thống với thực tế</p>
        </div>

        <FormSection icon={Calendar} title="THÔNG TIN PHIẾU KIỂM KÊ" className="mb-5">
          <div>
            <label className="block text-sm font-semibold mb-1.5">
              Ngày kiểm kê <span className="text-danger">*</span>
            </label>
            <input
              type="text"
              className="w-full modal-input bg-gray-50 text-sm text-muted cursor-not-allowed"
              value={date}
              readOnly
              disabled
            />
        ...
        <FormSection icon={Package} title="NGUYÊN LIỆU KIỂM KÊ" subtitle="Thêm ít nhất 1 nguyên liệu" className="mb-5">
          {errors.items && (
        ...
        <div className="flex items-center justify-end gap-3 mt-6 mb-8">
          ...
          <button
            ...
            onClick={handleSave}
          >
            {saving ? 'Đang lưu...' : 'Tạo phiếu kiểm kê'}
          </button>
        </div>
      </div>
    </PageContainer>
