...
const KPICard = ({ title, value, change, label, icon: Icon, isPositive, colorClass = "text-primary" }) => (
  <div className="card flex flex-col justify-between h-130px">
    <div className="flex justify-between items-start gap-4">
      <div className="min-w-0">
        <div className="text-muted text-sm font-semibold mb-1 uppercase tracking-wider truncate dashboard-stat-label">{title}</div>
        <div className={`text-2xl md:text-3xl font-bold ${colorClass} mt-1 truncate`}>{value}</div>
      </div>
      <div className="flex-shrink-0 bg-bg p-3 rounded-full">
        ...
        <div className="grid gap-4 md:gap-6 grid-cols-1 lg:grid-cols-3 w-full min-w-0">
          ...
          <div className="flex flex-col gap-6 lg:col-span-2 min-w-0">
            <div className="card h-full flex flex-col min-w-0">
              <div className="flex justify-between items-center mb-6 gap-4 flex-wrap">
                ...
                <select className="flex-shrink-0 h-36px">
                  <option>7 ngày gần nhất</option>
                  <option>Tháng này</option>
                </select>
              </div>
              <div className="w-full min-w-0 flex-1 h-380px">
          ...
          <div className="flex flex-col gap-4 md:gap-6 lg:col-span-1 min-w-0">
            <div className="card p-0 overflow-hidden min-w-0">
              <div className="p-4 flex items-center justify-between border-b bg-soft-gray">
              ...
              <div>
                {bestSellers.slice(0, 5).map((item, i) => (
                  <div key={item.id} className={`flex items-center justify-between p-4 hover-bg-gray-50 gap-2 ${i < 4 ? 'border-b' : ''}`}>
            ...
            <div className="card p-0 overflow-hidden min-w-0">
              <div className="p-4 flex items-center justify-between border-b bg-rose-50">
              ...
              <div className="p-4 flex flex-col gap-4 min-w-0">
                {inventoryWarnings.map(item => (
                  <div key={item.id} className="flex justify-between items-center gap-2 min-w-0">
                    ...
                    <span className="text-xs font-semibold px-2 py-1 rounded flex-shrink-0 bg-danger-light text-danger">{item.status}</span>
                  </div>
                ))}
