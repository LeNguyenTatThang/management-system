...
import { Search, Plus, Minus, Trash2, ArrowLeft, ShoppingBag, Coffee, Receipt, Banknote, Smartphone, X, CheckCircle, Tag } from 'lucide-react';
...
export default function PosLayout({
  ...
  const handleAction = () => {
    if (onAction) {
      onAction({ order, appliedCoupons, payment, total: grandTotal, clearOrder: () => { setOrder([]); setAppliedCoupons([]); } });
    } else {
      setDone(true);
      setTimeout(() => { setOrder([]); setDone(false); setAppliedCoupons([]); }, 2200);
    }
  };
    ...
    <div className="pos-container">
                ...
                <div className="flex items-center gap-2">
                  {SIZES.map(s => {
                    ...
                    return (
                      ...
                      >
                        <div className="text-base font-bold text-center">{s.label}</div>
                        <div className="text-xs font-semibold text-center">{fmt(adjusted)}</div>
                      </button>
                    );
                  })}
                </div>
      ...
      <div className="pos-left">
        <div className="pos-topbar">
          <button onClick={() => navigate(backPath)} className="pos-back-btn">
      ...
      <div className="pos-order-panel">
        ...
        <div className="pos-order-footer">
          ...
          <motion.button onClick={handleAction} disabled={order.length === 0}
          ...
          >
            {done ? (
              ...
              >
                <CheckCircle size={22} /> {actionLabel} thành công!
              </motion.span>
            ) : (
              <>{actionLabel} {order.length > 0 ? fmt(grandTotal) : ''}</>
            )}
          </motion.button>
        </div>
      </div>
