...
export default function ExportReceiptCreate() {
  ...
  const [items, setItems] = useState([
    { ingredientId: '', ingredientName: '', unit: '', requestedQuantity: '', currentStock: 0, note: '' },
  ]);
  ...
  const handleSave = async (status) => {
    if (!validate()) return;
    if (status === 'confirmed') {
      const stockErrors = items.filter(i => i.ingredientId).some(i => {
        const qty = parseFloat(i.requestedQuantity) || 0;
        return qty > i.currentStock;
      });
      if (stockErrors) {
        toast.error('Vui lòng kiểm tra lại số lượng xuất. Một số nguyên liệu không đủ tồn kho.');
        return;
      }
    }
    setSaving(true);
    try {
      ...
      toast.success(status === 'draft' ? 'Đã lưu nháp phiếu xuất' : 'Đã xác nhận phiếu xuất');
    ...
    <PageContainer>
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-muted text-sm mt-1">Tạo phiếu xuất nguyên liệu, hàng hóa ra khỏi kho</p>
        </div>
        ...
        <FormSection icon={Package} title="NGUYÊN LIỆU XUẤT" subtitle="Thêm ít nhất 1 nguyên liệu" className="mb-5">
          {errors.items && (
        ...
        {exportType === 'DISPOSAL' && (
          <FormSection icon={FileText} title="GHI CHÚ THÊM" className="mb-6">
            <textarea placeholder="Ghi chú thêm..." className="w-full modal-input" rows={2}
              value={note} onChange={e => setNote(e.target.value)} />
          </FormSection>
        )}

        <div className="flex items-center justify-end gap-3 mt-6 mb-8">
          <button type="button" className="btn btn-outline modal-btn px-6" onClick={() => navigate('/inventory/exports')}>Hủy</button>
          ...
          <button type="button" className={`btn btn-primary modal-btn px-6 flex items-center gap-2 ${saving ? 'opacity-70 cursor-not-allowed' : ''}`}
            disabled={saving} onClick={() => {
              if (!window.confirm('Xác nhận phiếu xuất này? Sau khi xác nhận, bạn có thể thực hiện xuất kho.')) return;
